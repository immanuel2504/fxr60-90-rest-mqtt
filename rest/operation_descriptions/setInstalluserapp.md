...
| Authentication type | `NONE` if the download server requires no credentials, or `BASIC` for username/password authentication (`authenticationOptions.username`, `authenticationOptions.password`). |
