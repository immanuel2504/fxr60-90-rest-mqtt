...
Usage
-----
  # On a lab PC that can reach the reader in lab_rest.json:
  python test_rest_api.py
  python test_rest_api.py --only "/cloud/status"

  # override host / password without editing the file
  ...
  python test_rest_api.py --flow ca-certificates --pem ./AmazonRootCA1.pem

  # show what would be called, contact nothing
  python test_rest_api.py --list --tier read --tier write

Reports land in `test script/reports/` (full run summary) and each test is also saved under:

- `reports/success/` — PASS
- `reports/failure/` — FAIL or WARN
"""
...
from _paths import LAB_REST as DEFAULT_CONFIG
from _paths import FAILURE_DIR, REPORT_DIR, REST_DIR, REST_EXAMPLES as EXAMPLES_DIR, SPEC, SUCCESS_DIR
...
# Paths come from _paths.py (this folder + the docs repo next to it).
...
def load_rest_config(path: str) -> dict:
...
def split_host(value: str) -> tuple[str, str | None]:
    """Allow host as IP or as https://ip."""
    raw = (value or "").strip()
    if raw.startswith("https://"):
        return raw[8:].split("/")[0], "https"
    if raw.startswith("http://"):
        return raw[7:].split("/")[0], "http"
...
def write_reports(results: list[dict], flow: list[dict], meta: dict) -> tuple[str, str]:
    ...
    for r in results:
        ...
        save_split_result(
            f"rest_{r.get('method')}_{r.get('url_path') or r.get('path')}",
            r,
        )
    ...
    if flow:
        ...
        for s in flow:
            ...
            save_split_result(
                f"rest_flow_{s.get('method')}_{s.get('path')}_{s.get('step')}",
                s,
            )
...
def parse_args(argv=None):
    ...
    ap.add_argument("--config", default=DEFAULT_CONFIG,
                    help="JSON with host/user/password (default: test script/lab_rest.json)")
    ap.add_argument("--host", default=None, help="reader IP or hostname (overrides config)")
    ap.add_argument("--user", default=None, help="admin username (env FXR_USER)")
    ap.add_argument("-p", "--password", default=None, help="admin password (env FXR_PASSWORD)")
    ap.add_argument("--token", default=None,
                    help="bearer token; skips login (env FXR_TOKEN). Tokens expire; prefer password.")
    ap.add_argument("--scheme", default=None, choices=["https", "http"])
    ...
    cfg = load_rest_config(args.config)
    host_in = args.host or os.environ.get("FXR_HOST") or cfg.get("host") or ""
    host, host_scheme = split_host(str(host_in))
    args.host = host
    args.user = args.user or os.environ.get("FXR_USER") or cfg.get("user") or "admin"
    args.password = args.password or os.environ.get("FXR_PASSWORD") or cfg.get("password")
    args.token = args.token or os.environ.get("FXR_TOKEN")
    if not args.token and not args.password:
        args.token = cfg.get("token")
    args.scheme = args.scheme or host_scheme or cfg.get("scheme") or "https"
    ...
    if not args.list:
        if not args.host:
            ap.error("host missing: set test script/lab_rest.json or pass --host")
        if not args.token and not args.password:
            ap.error("password missing: set lab_rest.json or pass --password / --token")
    return args
...
def main(argv=None) -> int:
    ...
    if args.token:
        token = args.token
        print(paint(f"Using supplied bearer token against {base}", "head", color))
    else:
